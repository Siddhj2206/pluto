...
@test "00-image-info: writes Bluefin-compatible image metadata" {
    ...
    [ "$status" -eq 0 ]
    [ "$(json_field image-name)" = "pluto" ]
    [ "$(json_field image-vendor)" = "projectbluefin" ]
    [ "$(json_field image-ref)" = "ostree-image-signed:docker://ghcr.io/projectbluefin/pluto" ]
...
@test "00-image-info: does not create a flavor field from the image name" {
    export IMAGE_NAME="pluto-nvidia"
    run_script
    [ "$status" -eq 0 ]
    [ "$(json_field image-ref)" = "ostree-image-signed:docker://ghcr.io/projectbluefin/pluto-nvidia" ]
...
@test "00-image-info: updates existing base os-release identity" {
    ...
    grep -q '^VARIANT_ID="pluto"$' "${OS_RELEASE}"
    grep -q '^PRETTY_NAME="pluto (Version: 44.20260907.1)"$' "${OS_RELEASE}"
    grep -q '^NAME="pluto"$' "${OS_RELEASE}"
    grep -q '^VERSION="44.20260907.1 (silverblue)"$' "${OS_RELEASE}"
    grep -q '^OSTREE_VERSION="44.20260907.1"$' "${OS_RELEASE}"
    grep -q '^IMAGE_ID="pluto"$' "${OS_RELEASE}"
...
@test "00-image-info: derives GitHub URLs from image identity" {
    ...
    grep -q '^HOME_URL="https://github.com/projectbluefin/pluto"$' "${OS_RELEASE}"
    grep -q '^DOCUMENTATION_URL="https://github.com/projectbluefin/pluto/blob/main/README.md"$' "${OS_RELEASE}"
    grep -q '^SUPPORT_URL="https://github.com/projectbluefin/pluto/issues"$' "${OS_RELEASE}"
    grep -q '^BUG_REPORT_URL="https://github.com/projectbluefin/pluto/issues/new"$' "${OS_RELEASE}"
    grep -q '^ID_LIKE="fedora"$' "${OS_RELEASE}"
}

@test "00-image-info: permits explicit URL overrides without changing base identity" {
    export HOME_URL="https://pluto.example"
    export DOCUMENTATION_URL="https://docs.pluto.example"
    export SUPPORT_URL="https://support.pluto.example"
    export BUG_REPORT_URL="https://bugs.pluto.example"
    run_script
    [ "$status" -eq 0 ]

    grep -q '^PRETTY_NAME="pluto (Version: stable)"$' "${OS_RELEASE}"
    grep -q '^NAME="pluto"$' "${OS_RELEASE}"
    grep -q '^ID_LIKE="fedora"$' "${OS_RELEASE}"
    grep -q '^HOME_URL="https://pluto.example"$' "${OS_RELEASE}"
    grep -q '^DOCUMENTATION_URL="https://docs.pluto.example"$' "${OS_RELEASE}"
    grep -q '^SUPPORT_URL="https://support.pluto.example"$' "${OS_RELEASE}"
    grep -q '^BUG_REPORT_URL="https://bugs.pluto.example"$' "${OS_RELEASE}"
}
...
@test "00-image-info: escapes JSON values" {
    ...
    export IMAGE_NAME='pluto"test'
    run_script
    [ "$status" -eq 0 ]

    run python3 -m json.tool "${IMAGE_INFO_JSON}"
    [ "$status" -eq 0 ]
    [ "$(json_field image-name)" = 'pluto"test' ]
}
...
@test "00-image-info: replaces an existing key whose value contains sed metacharacters" {
    export HOME_URL="https://pluto.example/?a=1&b=2|c"
    run_script
    [ "$status" -eq 0 ]

    grep -qF 'HOME_URL="https://pluto.example/?a=1&b=2|c"' "${OS_RELEASE}"
}
