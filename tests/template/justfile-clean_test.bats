...
@test "clean: removes top-level entries whose name contains _build" {
	mkdir -p "${SANDBOX}/pluto_build/layer"
	touch "${SANDBOX}/pluto_build/layer/blob" \
...
@test "sudo-clean: propagates the sudoif failure instead of cleaning unprivileged" {
	...
	mkdir -p "${SANDBOX}/pluto_build" "${SANDBOX}/output"
	touch "${SANDBOX}/pluto_build/blob" "${SANDBOX}/output/manifest"
