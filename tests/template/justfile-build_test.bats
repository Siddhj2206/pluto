...
@test "build: passes the image identity build args bootc relies on" {
    ...
    [[ "${args}" == *"--build-arg IMAGE_NAME=pluto"* ]]
...
@test "build: falls back to the GitHub repository owner for the vendor" {
    ...
    GITHUB_REPOSITORY_OWNER="acme-org" run_just build pluto stable
    [ "$status" -eq 0 ]

    local args
    args="$(podman_build_args)"
    [[ "${args}" == *"--build-arg IMAGE_VENDOR=acme-org"* ]]
    [[ "${args}" == *"--cache-from ghcr.io/acme-org/pluto"* ]]
}

@test "build: honours IMAGE_VENDOR and UBLUE_IMAGE_TAG overrides" {
    IMAGE_VENDOR="acme" UBLUE_IMAGE_TAG="pinned" run_just build pluto stable
    [ "$status" -eq 0 ]
    local args
    args="$(podman_build_args)"
...
@test "build: strips a local registry prefix from the image identity" {
    ...
    run_just build localhost/pluto stable
    [ "$status" -eq 0 ]
    local args
    args="$(podman_build_args)"
    [[ "${args}" == *"--build-arg IMAGE_NAME=pluto"* ]]
    [[ "${args}" == *"--tag localhost/pluto:stable"* ]]
}

@test "build: forwards GITHUB_TOKEN as a build secret when set" {
    GITHUB_TOKEN="s3cret" run_just build pluto stable
    [ "$status" -eq 0 ]
...
@test "build: forwards explicit Containerfile metadata overrides" {
    IMAGE_DESC="Custom image" IMAGE_LOGO_URL="https://example.com/logo.svg" IMAGE_KEYWORDS="bootc,custom" IMAGE_REF="feature" run_just build pluto stable
    [ "$status" -eq 0 ]
    local args
    args="$(podman_build_args)"
...
@test "build: reads the layer cache but never writes it by default" {
    ...
    local args
    args="$(podman_build_args)"
    [[ "${args}" == *"--cache-from ghcr.io/projectbluefin/pluto"* ]]
    [[ "${args}" != *"--cache-to"* ]]
}

@test "build: writes the layer cache when REGISTRY_CACHE_WRITE=1" {
    REGISTRY_CACHE_WRITE=1 run_just build pluto stable
    [ "$status" -eq 0 ]
    [[ "$(podman_build_args)" == *"--cache-to ghcr.io/projectbluefin/pluto"* ]]
}
...
@test "tag-images: rejects an empty default tag" {
    run_just tag-images pluto "" "one two"
    [ "$status" -eq 1 ]
    [[ "$output" == *"Usage: just tag-images"* ]]
}

@test "tag-images: rejects an empty tag list" {
    run_just tag-images pluto stable ""
    [ "$status" -eq 1 ]
    [[ "$output" == *"Usage: just tag-images"* ]]
}

@test "tag-images: untags the default tag before re-tagging by image id" {
    run_just tag-images pluto stable "latest 44"
    [ "$status" -eq 0 ]

    mapfile -t calls <"${PODMAN_LOG}"
    [ "${calls[0]}" = "inspect localhost/pluto:stable" ]
    [ "${calls[1]}" = "untag localhost/pluto:stable" ]
    [ "${calls[2]}" = "tag sha256:deadbeef pluto:latest" ]
    [ "${calls[3]}" = "tag sha256:deadbeef pluto:44" ]
}

@test "tag-images: re-applies the default tag so local lookups still resolve" {
    run_just tag-images pluto stable "latest"
    [ "$status" -eq 0 ]
    [[ "$(tail -n1 "${PODMAN_LOG}")" = "tag sha256:deadbeef pluto:stable" ]]
    [[ "$output" == *"Tagged pluto with: latest"* ]]
}
