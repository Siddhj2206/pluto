...
@test "tag-images: resolves the image id from the localhost-qualified default tag" {
	run_tag_images pluto stable "stable-42 latest"
	[ "$status" -eq 0 ]
	podman_calls | grep -Fxq 'inspect localhost/pluto:stable'
}

@test "tag-images: untags the localhost default reference before re-tagging" {
	run_tag_images pluto stable "stable-42"
	[ "$status" -eq 0 ]
	...
	podman_calls | grep -Fxq 'untag localhost/pluto:stable'
}

@test "tag-images: applies every tag in the whitespace-separated list" {
	run_tag_images pluto stable "stable-42 stable-42.20250101 latest"
	[ "$status" -eq 0 ]
	podman_calls | grep -Fxq 'tag sha256:deadbeef pluto:stable-42'
	podman_calls | grep -Fxq 'tag sha256:deadbeef pluto:stable-42.20250101'
	podman_calls | grep -Fxq 'tag sha256:deadbeef pluto:latest'
}

@test "tag-images: re-applies the default tag so local lookups keep working" {
	run_tag_images pluto stable "latest"
	[ "$status" -eq 0 ]
	podman_calls | grep -Fxq 'tag sha256:deadbeef pluto:stable'

	# The default tag is restored last, after every alias tag.
	local last_tag
	last_tag="$(grep '^tag ' "${PODMAN_LOG}" | tail -n1)"
	[ "${last_tag}" = 'tag sha256:deadbeef pluto:stable' ]
}
...
@test "tag-images: collapses repeated whitespace in the tag list" {
	run_tag_images pluto stable "  latest   stable-42  "
	[ "$status" -eq 0 ]
...
@test "tag-images: reports the tags it applied" {
	run_tag_images pluto stable "latest stable-42"
	[ "$status" -eq 0 ]
	[[ "$output" == *"Tagged pluto with: latest stable-42"* ]]
}
...
STUB
	...
	PODMAN=docker-shim run_tag_images pluto stable "latest"
	[ "$status" -eq 0 ]
	podman_calls | grep -Fxq 'shim inspect localhost/pluto:stable'
	podman_calls | grep -Fxq 'shim tag sha256:cafe pluto:latest'
